...
?>
		<tr>
            <td><a href="<?php echo "/profile.php?prof=" . $running_variable['username']; ?>"   ><?php echo $running_variable['username']; ?></a></td>
            ...
            if ($running_variable['username'] != $_SESSION['name']){
                if (!in_array($running_variable['username'], array_column($friendsAlready, "username"))){
                    if (isset($_SESSION['name'])){
                        echo '<a class="btn btn-primary" role="button" href="/addFriend.php?prof=' . $running_variable['username'] . '"> Add ' .  $running_variable['username'] . ' as Friend </a>';?></td>
                   <?php 
                    }
                    else{
